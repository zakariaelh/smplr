...
class  _audio_pre_():
    ...
    def _path_audio_(self, music_file ,ins_root=None,vocal_root=None):
        ...
        t2 = time.time()
        print('Time before inference', t2 - t1)
        t1 = time.time()
        with no_grad():
        ...
        if (vocal_root is not None):
            ...
            t2 = time.time()
            print('post processing ', t2 - t1)
            file_name_without_mp3 = name.rsplit('.mp3', 1)[0]
            vocals_filename = os.path.join(vocal_root , 'vocal_{}.wav'.format(file_name_without_mp3))
            t1 = time.time()
            wavfile.write(vocals_filename, self.mp.param['sr'], (np.array(wav_vocals)*32768).astype("int16"))
            # wavfile.write(buffer, self.mp.param['sr'], (np.array(wav_vocals)*32768).astype("int16"))
            t2 = time.time()
            print('saving to buffer time', t2 - t1)
